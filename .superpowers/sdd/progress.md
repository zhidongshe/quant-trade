...
Whole-branch fix wave: complete (commit 8396fbd — Important #1 realized_pnl after passorder + #3 ACCOUNT_ID guard + #4 test code-form tighten + #6 rejection-path test; Minor #1 Windows path + #2 dead ranked_candidates + #4 Account cleanup + #5 type hints; golden 2020 baseline UNCHANGED, 111+3 E2E all pass)

Task 1: complete (commit 39af284, implemented inline after subagent found target file is untracked; focused config test passed)
Task 2: complete (commit ea16f21, implemented inline after subagent blocked on isolated worktree missing Task 1 outputs; focused path test passed)
Task 3: complete (commit b0bf5ea via cherry-pick from worktree 8b3e26f, focused reconcile tests passed)
Task 4: complete (commit ac0a42e via cherry-pick from worktree e6ca3ab, focused guard tests passed)
Task 5: complete (commit 50aae57, implemented inline after worktree caveat; pending-order state test passed)
Task 6: complete (commit 8d230cd, implemented inline after worktree blocked on missing prior-task files; buy confirmation test passed)
Task 7: complete (commit f8cbd59, implemented inline after worktree blocked on missing prior-task files; sell confirmation test passed)
Task 8: complete (commit 444c166, implemented inline after worktree blocked on missing prior-task files; runtime guard test passed)
Task 9: complete (commit e9484d5, implemented inline after worktree blocked on missing prior-task files; stale pending order test passed)
Task 10: complete (commit a840927, implemented inline after worktree blocked on missing prior-task files; smoke test + focused suite 13 passed)
