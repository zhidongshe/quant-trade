...
策略逻辑:
1. 股票池: 沪深300成分股，每日更新
2. 入场: 四因子共振 (60日线上 + 5日>20日 + MACD>0 + 放量20%)
3. 卖出: 5%硬止损 / 跌破20日线 / 盈利10%后跟踪止盈(回落8%)
4. 仓位: 最多5只，每只20%
5. v2改进: 截面Min-Max归一化打分 + 最大回撤/胜率/盈亏比统计 + 总资产扣成本
...
def check_buy_signal(prices, volumes):
    ...
    if np.isnan(vol_ma20[-1]) or volumes[-1] <= vol_ma20[-1] * 1.2:
        return False
...
def score_stock(prices, volumes):
    """对满足四因子条件的股票打分，返回原始分项字典；不满足条件返回None"""
    ...
    dif, dea, hist = macd(prices)
    # v2: MACD相对DEA的强度，而非除以价格
    macd_score = hist[-1] / abs(dea[-1]) if abs(dea[-1]) > 1e-9 else 0.0
...
def init(ContextInfo):
    ...
    ContextInfo.gross_profit = 0.0    # 总盈利金额（仅盈利交易）
    ContextInfo.gross_loss = 0.0      # 总亏损金额（仅亏损交易，取绝对值）
    ContextInfo.max_drawdown = 0.0    # 最大回撤金额
    ContextInfo.peak_assets = ContextInfo.capital  # 总资产峰值
...
def handlebar(ContextInfo):
    ...
    if len(ContextInfo.positions) > 0:
        for stockcode, pos in list(ContextInfo.positions.items()):
            ...
            if should_sell:
                ...
                if realized > 0:
                    ContextInfo.win_count = getattr(ContextInfo, 'win_count', 0) + 1
                    ContextInfo.gross_profit = getattr(ContextInfo, 'gross_profit', 0.0) + realized
                else:
                    ContextInfo.lose_count = getattr(ContextInfo, 'lose_count', 0) + 1
                    ContextInfo.gross_loss = getattr(ContextInfo, 'gross_loss', 0.0) + abs(realized)
    ...
    for stockcode in buy_universe:
        ...
        scores = score_stock(prices_arr, volumes_arr)
        if scores is not None:
            raw_scores.append((stockcode, scores))

    # v2: 截面 Min-Max 归一化
    scored = []
    if len(raw_scores) > 0:
        keys = ['trend', 'ma_spread', 'macd', 'volume']
        mins = {k: min(s[k] for _, s in raw_scores) for k in keys}
        maxs = {k: max(s[k] for _, s in raw_scores) for k in keys}
        weights = {'trend': 0.30, 'ma_spread': 0.25, 'macd': 0.25, 'volume': 0.20}
        for stockcode, s in raw_scores:
            norm_total = 0.0
            for k in keys:
                rng = maxs[k] - mins[k]
                if rng > 1e-9:
                    norm_val = (s[k] - mins[k]) / rng
                else:
                    norm_val = 0.5
                norm_total += norm_val * weights[k]
            scored.append((stockcode, norm_total))
...
def _log_status(ContextInfo, current_date):
    ...
    # v2: 预计算交易统计
    trade_count = getattr(ContextInfo, 'trade_count', 0)
    win_count = getattr(ContextInfo, 'win_count', 0)
    lose_count = getattr(ContextInfo, 'lose_count', 0)
    win_rate = (win_count / trade_count * 100) if trade_count > 0 else 0.0
    gross_profit = getattr(ContextInfo, 'gross_profit', 0.0)
    gross_loss = getattr(ContextInfo, 'gross_loss', 0.0)
    ...
    if len(holdings) == 0 and len(sold_records) == 0:
        ...
        # v2: 更新最大回撤
        if total_assets > ContextInfo.peak_assets:
            ContextInfo.peak_assets = total_assets
        drawdown = ContextInfo.peak_assets - total_assets
    ...
    # 2. 打印今日卖出，并汇总盈亏
    sold_pnl_sum = 0.0  # v2: 单独记录今日卖出盈亏，用于修正总资产
    ...
    if total_assets is None:
        ...
        holding_pnl_only = total_pnl - sold_pnl_sum
        total_assets = ContextInfo.capital + realized_pnl + holding_pnl_only - total_cost_acc
